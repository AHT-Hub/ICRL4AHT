"""Episode execution utilities for evaluation.

This module provides functions for running evaluation episodes with an ego agent
and a partner agent using JAX's scan for efficient episode execution.

Note: Does not currently support actors that require aux_obs.
"""
...
def run_single_episode(rng, env, agent_0_param, agent_0_policy,
    ...
    act_0, hstate_0 = agent_0_policy.get_action(
        ...
        obs=init_obs["agent_0"][None, None, ...],  # Preserve spatial dims: (1, 1, H, W, C) or (1, 1, obs_dim)
        done=init_done["agent_0"].reshape(1, 1),
        avail_actions=avail_actions_0.reshape(1, 1, -1),  # (1, 1, action_dim)
    ...
    act_1, hstate_1 = agent_1_policy.get_action(
        ...
        obs=init_obs["agent_1"][None, None, ...],  # Preserve spatial dims: (1, 1, H, W, C) or (1, 1, obs_dim)
        done=init_done["agent_1"].reshape(1, 1),
        avail_actions=avail_actions_1.reshape(1, 1, -1),  # (1, 1, action_dim)
    ...
    def scan_step(carry, _):
        def take_step(carry_step):
            ...
            act_0, hstate_0_next = agent_0_policy.get_action(
                params=agent_0_param,
                obs=obs["agent_0"][None, None, ...],  # Preserve spatial dims: (1, 1, H, W, C) or (1, 1, obs_dim)
            ...
            act_1, hstate_1_next = agent_1_policy.get_action(
                params=agent_1_param,
                obs=obs["agent_1"][None, None, ...],  # Preserve spatial dims: (1, 1, H, W, C) or (1, 1, obs_dim)
