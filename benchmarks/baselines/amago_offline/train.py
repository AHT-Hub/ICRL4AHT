#!/usr/bin/env python3
"""AMAGO-offline Training Script for Overcooked V2.

Trains an AMAGO-style offline sequence model on collected learning histories.
Uses timestep encoding + GRU trajectory encoding for long-range memory.

Usage:
    python -m benchmarks.baselines.amago_offline.train \
        --h5_path datasets/histories.h5 \
        --index_path datasets/histories_index.jsonl \
        --out_dir outputs/amago_offline
"""
...
from benchmarks.baselines.amago_offline.model import (
    AMAGOOfflineConfig, AMAGOOfflineModel,
    create_amago_offline_model, count_parameters,
)
from benchmarks.baselines.amago_offline.buffer import sample_amago_batch, AMAGOBatch
...
class TrainConfig:
    ...
    out_dir: str = "outputs/amago_offline"
    ...
    def to_model_config(self) -> AMAGOOfflineConfig:
        return AMAGOOfflineConfig(
            obs_shape=self.obs_shape,
            num_actions=self.num_actions,
            embedding_dim=self.embedding_dim,
            hidden_dim=self.hidden_dim,
            tstep_hidden_dim=self.tstep_hidden_dim,
            traj_encoder=self.traj_encoder,
            traj_hidden_dim=self.traj_hidden_dim,
            num_traj_layers=self.num_traj_layers,
...
class PrefetchDataLoader:
    ...
    def _worker(self):
        try:
            while not self.stop_event.is_set():
                batch = sample_amago_batch(
                    self.store, self.rng, self.batch_size, self.seq_len,
    ...
    def get_batch(self, timeout: float = 300.0) -> AMAGOBatch:
...
def create_train_state(config: TrainConfig, model: AMAGOOfflineModel, params: Any) -> TrainState:
...
def batch_to_tuple(batch: AMAGOBatch, use_teammate_actions: bool = False) -> Tuple:
    base = (
        jnp.array(batch.obs),
        jnp.array(batch.prev_actions),
        jnp.array(batch.prev_rewards),
        jnp.array(batch.target_actions),
        jnp.array(batch.dones, dtype=jnp.float32),
        jnp.array(batch.time_idxs),
...
def _train_step_impl(
    ...
    if use_teammate_actions:
        obs, prev_actions, prev_rewards, target_actions, dones, time_idxs, attention_mask, prev_teammate_actions = batch_data
    else:
        obs, prev_actions, prev_rewards, target_actions, dones, time_idxs, attention_mask = batch_data
...
def _eval_step_impl(state, batch_data, num_actions, use_teammate_actions):
    ...
    logits = state.apply_fn(
        state.params, obs, prev_actions, prev_rewards, dones, time_idxs,
        attention_mask=attention_mask,
        prev_teammate_actions=prev_teammate_actions,
        train=False,
    )
...
def evaluate(state, store, config, rng):
    ...
    for _ in range(config.eval_batches):
        batch = sample_amago_batch(
            store, rng, config.eval_batch_size, config.seq_len,
...
def train(config: TrainConfig):
    ...
    log.info(f"Training AMAGO-offline model")
    ...
    log.info(f"  Traj encoder: {config.traj_encoder}")
    log.info(f"  Traj hidden dim: {config.traj_hidden_dim}")
    log.info(f"  Traj layers: {config.num_traj_layers}")
    ...
    model, params = create_amago_offline_model(model_config)
    ...
    try:
        for step in range(start_step, config.num_steps):
            if data_loader is not None:
                ...
                batch = sample_amago_batch(
                    store, rng, config.batch_size, config.seq_len,
                    include_teammate_actions=config.use_teammate_actions,
                )
...
def parse_args() -> TrainConfig:
    parser = argparse.ArgumentParser(description="Train AMAGO-offline model")
    ...
    parser.add_argument("--tstep_hidden_dim", type=int, default=256)

    parser.add_argument("--traj_encoder", type=str, default="gru", choices=["gru", "transformer"])
    parser.add_argument("--traj_hidden_dim", type=int, default=256)
    parser.add_argument("--num_traj_layers", type=int, default=2)
    ...
    parser.add_argument("--out_dir", type=str, default="outputs/amago_offline")
    ...
    return TrainConfig(
        ...
        tstep_hidden_dim=args.tstep_hidden_dim,
        traj_encoder=args.traj_encoder,
        traj_hidden_dim=args.traj_hidden_dim,
        num_traj_layers=args.num_traj_layers,
